...
import { Link } from 'react-router-dom';
 ...
 const CustomProductComponent = ({ product, cardHeight = '400px', Card2xl = '500px' }) => {
    const dynamicHeightClass = `min-h-[${cardHeight}]`;
  return (
    <>
    <Link to={`/${product.id}/product_view`}>
      <img
        src={product.image || 'default-image.jpg'} // Use `image` from the product or a default placeholder
        alt="product img"
        className="w-full min-h-[200px] max-h-[400px]"
      />
      <div className="p-2 flex gap-2 flex-col">
        <p className="text-sm font-bold text-gray-500">
          <span className="text-red-600">{product.sale_price || product.regular_price}</span>{' '}
          {product.sale_price && <span className="line-through">{product.regular_price}</span>}
        </p>
        ...
        <h6 className="text-gray-400 text-xs">
          <i className="fa-solid fa-box mr-2"></i> {product.stock_status || 'Unknown'}
        </h6>
      </div>
    </Link>
  </>
